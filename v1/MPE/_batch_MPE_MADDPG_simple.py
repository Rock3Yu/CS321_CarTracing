...
TENSORBOARD_PATH = './logs/random_test/simple_30000_60c/'
...
class MADDPG_Policy:
    def __init__(self, mode,num_police, num_criminal,num_escape,gpu=False):  # mode = Train,Test
        ...
        for i in range(self.num_agent):
            ...
            self.optimizer_actors[i] = optim.Adam(self.actors[i].parameters(), lr=0.98)  # 学习率1.直接更新，momentum动量
            self.optimizer_critics[i] = optim.Adam(self.critics[i].parameters(), lr=0.98)
        ...
        for epoch in range(num_epoch):  # the epoch is a training process 1个epoch等于使用训练集中的全部样本训练一次，通俗的讲epoch的值就是整个数据集被轮几次。
            ...
            for step in range(cycles):  # the step is a step in max-steps
                ...
                for i in range(self.num_agent):
                    ...
                    if  self.gpu:
                        action = self.actors[i](torch.as_tensor( obs,device='cuda:0',dtype=torch.float32)).detach().cpu().numpy()
                    else:
                        action = self.actors[i](torch.as_tensor(obs,dtype=torch.float32)).detach().numpy()
                    
                            # 即返回一个新的tensor，从当前计算图中分离下来。 但是仍指向原变量 的 存放位置，不能用于计算梯度
                    if  epoch < 3000 and self.mode==0:
                ...
                if self.mode==0 :  # Train#########################################
                    ...
                    cul_step=epoch*cycles+step

                    if  cul_step % update_interval ==0 and epoch>3000:
                        for i in range(self.num_agent):
                                ...
                                for j in range(self.num_agent):
                                    ...
                                    if  self.gpu:
                                        action_new = self.actors_target[i](torch.as_tensor(obs_new,device='cuda:0',dtype=torch.float32)).detach().cpu().numpy()
                                    else:
                                        action_new = self.actors_target[j](torch.as_tensor((obs_new),dtype=torch.float32)).detach().numpy() #对每个智体确定他们的下一步动作,根据A_target,μ'
                                ...
                                actions_new=np.transpose(np.array(actions_new,dtype=np.float32),(1,0,2))
                                if self.gpu:

                                    Q = self.critics[i](torch.as_tensor(obs_agents,device='cuda:0',dtype=torch.float32)
                                        , torch.as_tensor(actions,device='cuda:0',dtype=torch.float32))  #找到非冻结的Q函数根据obs和actions
                                    Q_target = self.critics_target[i](torch.as_tensor(obs_new_agents,device='cuda:0',dtype=torch.float32)
                                        , torch.as_tensor(actions_new,device='cuda:0',dtype=torch.float32)) #μ'目标Q函数，即冻结的Q函数
                                    ...
                                    loss_actor = -torch.mean(self.critics[i](torch.as_tensor(obs_agents,device='cuda:0',dtype=torch.float32)
                                        , torch.as_tensor(actions,device='cuda:0',dtype=torch.float32))) #求critic网络的均值，返回的是标量
                                    loss_actor.backward() # 反向传播计算梯度
                                    self.optimizer_actors[i].step() #将μ函数（策略函数）的参数进行更新，更新μ
                                else:
                                    Q = self.critics[i](torch.as_tensor(obs_agents,dtype=torch.float32)
                                        , torch.as_tensor(actions,dtype=torch.float32))  #找到非冻结的Q函数根据obs和actions
                                    Q_target = self.critics_target[i](torch.as_tensor(obs_new_agents,dtype=torch.float32)
                                        , torch.as_tensor(actions_new,dtype=torch.float32)) #μ'目标Q函数，即冻结的Q函数
                                    ...
                                    loss_actor = -torch.mean(self.critics[i](torch.as_tensor(obs_agents,dtype=torch.float32)
                                        , torch.as_tensor(actions,dtype=torch.float32))) #求critic网络的均值，返回的是标量
                                    loss_actor.backward() # 反向传播计算梯度
                                    self.optimizer_actors[i].step() #将μ函数（策略函数）的参数进行更新，更新μ
    ...
    class Actor(nn.Module):
        ...
        def forward(self, obs):   #############问题在这里
            # obs = torch.tensor(np.array(obs.cpu()), dtype=torch.float32,device='cuda:0').flatten()
            if len(obs.shape)>1:
                obs = obs.flatten(1)
    ...
    class Memory:
        def __init__(self, size,obs_dim,action_dim,agent_num):
            self.size = size
            self.buffer_obs =np.zeros((size,agent_num,obs_dim),dtype=np.float32)
            self.buffer_actions = np.zeros((size,agent_num,action_dim),dtype=np.float32)
            self.buffer_rewards = np.zeros((size,agent_num),dtype=np.float32)
            self.buffer_obs_new = np.zeros((size,agent_num,obs_dim),dtype=np.float32)
            self.cur_idx = 0
            self.total_cnt = 0
...
if __name__ == '__main__':
    ...
    policy = MADDPG_Policy('test',0,3,1,gpu)
    policy.act(10, 60, 512,50, ifRender=False, load=False)  # 训练轮次，每轮步数，每步Buffer里面挑选的个数，渲染，加载已保存得网络
    if gpu:
        printReward(policy)
