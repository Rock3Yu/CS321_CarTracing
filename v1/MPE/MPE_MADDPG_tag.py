...
import random
import pettingzoo.mpe.simple_tag.simple_tag as simple_tag
from torch.utils.tensorboard import SummaryWriter
import os
TENSORBOARD_PATH = './logs/exp_tag_2000/'
...
class MADDPG_Policy:
    def __init__(self, mode,num_police, num_criminal,num_escape,gpu=False):  # mode = Train,Test
        ...
        for i in range(self.num_agent):
            if i<num_criminal:
                self.actors[i] = self.Actor(num_othernotadv=num_police,num_agent=self.num_agent,num_entity=self.num_obstacles)
                self.actors_target[i] = self.Actor(num_othernotadv=num_police,num_agent=self.num_agent,num_entity=self.num_obstacles)
                self.critics[i] = self.Critic(num_othernotadv=num_police,num_agent=self.num_agent,num_entity=self.num_obstacles)
                self.critics_target[i] = self.Critic(num_othernotadv=num_police,num_agent=self.num_agent,num_entity=self.num_obstacles)
            else:#good
                self.actors[i] = self.Actor(num_othernotadv=num_police-1,num_agent=self.num_agent,num_entity=self.num_obstacles)
                self.actors_target[i] = self.Actor(num_othernotadv=num_police-1,num_agent=self.num_agent,num_entity=self.num_obstacles)
                self.critics[i] = self.Critic(num_othernotadv=num_police-1,num_agent=self.num_agent,num_entity=self.num_obstacles)
                self.critics_target[i] = self.Critic(num_othernotadv=num_police-1,num_agent=self.num_agent,num_entity=self.num_obstacles)
    ...
    def act(self,num_epoch, cycles, train_batch,ifRender,ifload): 
        if self.gpu:
            self.importcuda()
        ...
        env = simple_tag.env( num_good=self.num_police,num_adversaries=self.num_criminal,num_obstacles=self.num_obstacles,max_cycles=cycles*4,continuous_actions= True)
        ...
        for epoch in range(num_epoch):  # the epoch is a training process 1个epoch等于使用训练集中的全部样本训练一次，通俗的讲epoch的值就是整个数据集被轮几次。
            ...
            for step in range(cycles):  # the step is a step in max-steps
                ...
                action_idx = step % 20
                if (step+1) % 50 == 0:
                    print(f'training tag...  step={step+1}/{cycles}' if self.mode==0 else f'testing...  step={step+1}/{cycles}')
                if ifRender:
                    env.render()
            ...
            self.writer.add_scalar(tag_t,self.touches[epoch],epoch)
    def to1D(self,list2):
        list1=[]
        for t in list2:
            list1.append(t[0])
    ...
    def save(self):
        for i in range(self.num_agent):
            ...
            torch.save(self.actors[i].state_dict(), f'./data_tag/actors/{i}')
            torch.save(self.actors_target[i].state_dict(), f'./data_tag/actors_target/{i}')
            torch.save(self.critics[i].state_dict(), f'./data_tag/critics/{i}')
            torch.save(self.critics_target[i].state_dict(), f'./data_tag/critics_target/{i}')
    ...
    class Actor(nn.Module):
        def __init__(self,num_othernotadv,num_agent,num_entity):
            super().__init__()
            input1=4+num_othernotadv*2+(num_agent-1)*2+num_entity*2
    ...
    class Critic(nn.Module):
        def __init__(self,num_othernotadv,num_agent,num_entity):
            super().__init__()
            input1=4+num_othernotadv*2+(num_agent-1)*2+num_entity*2
            # self.fc1_1 = nn.Linear(input1, input1)
            # self.fc1_2 = nn.Linear(input1, input1//2)
            # self.fc2_1 = nn.Linear(num_agent * 5,num_agent*5)
            # self.fc2_2 = nn.Linear(2*num_agent*5, num_agent*5)
            # input2=input1//2+num_agent*5
            self.fc3_1 = nn.Linear(input1+num_agent*5,128)
...
def printReward(policy):
    print_log = open("/home/maddpg/v1/MPE/reward.txt",'w')
...
def printTouch(policy):
    print_log = open("/home/maddpg/v1/MPE/touches.txt",'w')
...
if __name__ == '__main__':
    ...
    policy = MADDPG_Policy('Train',1, 3,1,gpu=True)
    policy.act(4000, 170, 5,ifRender=False,ifload=False)  # 训练轮次，每轮步数，每步Buffer里面挑选的个数，渲染
    printReward(policy)
    printTouch(policy)
