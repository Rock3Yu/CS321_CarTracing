...
from Car_Tracing_Core import Police, Criminal
from map_manager import map_encode
...
class policy:
    ...
    def explore(self):
        for _ in range(self.args.explore_epoch):
            ...
            for _ in range(self.args.num_step):
                self.interact(False, 'random', 'random', None, None, 'train')

    def train(self, render):
        self.temp_rewards = {}
        for step in range(self.args.num_step):
            self.interact(render, 'random', 'net', self.total_epoch, step, 'train')
    ...
    def test(self, render, epoch):
        self.temp_rewards = {}
        for step in range(self.args.num_step):
            self.interact(render, 'random', 'net', epoch, step, 'test')
            if self.args.test_mode:
                for agent in self.env.world.agents: self.benchmark(agent, step)
        for key in self.temp_rewards.keys(): self.temp_rewards[key] /= self.args.num_step
        self.writer.add_scalars('test-rew-by-epoch', self.temp_rewards, epoch)

    def interact(self, render, action_p, action_c, epoch, step, mode):
        ...
        with torch.no_grad():
            for agent in self.env.world.agents:
                ...
                action = self.select_action(action_p if isinstance(agent, Police) else action_c, name, obs, image)
    ...
    def select_action(self, act_type, name, obs, image):
        action = torch.from_numpy(np.random.uniform(-1, 1, self.env.action_spaces[name].shape[0])).type(self.FT) if self.env.metadata['name']=='car_tracing' else torch.from_numpy(np.random.uniform(-1, 1, 5)).type(self.FT) 
        if act_type == 'random': return action
        if act_type == 'net': return self.args.noise * action + \
                    (1 - self.args.noise) * self.actor[name](obs, image).detach().type(self.FT)
        ...
        with torch.no_grad():
            ...
            for i in range(self.args.batch_size):
                ...
                action_new[i] = np.concatenate([self.actor_target[name]\
                    (torch.tensor(batch[name]['obs_new'][i]).type(self.FT), images[i])\
                        .detach().type(self.FT).cpu() for name in batch])
    ...
    def update(self):
        ...
        for agent in self.env.world.agents:
            name = agent.name
            if self.env.metadata['name']=='car_tracing':
                if isinstance(agent, Criminal if self.args.train_police else Police):
                    act_shape += self.env.action_spaces[name].shape[0]
                    continue
            ...
            Q = self.critic[name](obs, action, images).type(self.FT)
            loss_c = nn.MSELoss()(Q, Q_target).type(self.FT)
            self.critic_optim[name].zero_grad()
            loss_c.backward()

            self.critic_optim[name].step()
            old_action = torch.tensor(np.copy(action.cpu())).type(self.FT)
            old_action[:, act_shape:act_shape+self.env.action_spaces[name].shape[0]] = \
                self.actor[name](torch.tensor(batch[name]['obs']).type(self.FT), images.type(self.FT))
            act_shape += self.env.action_spaces[name].shape[0]
            loss_a = -self.critic[name](obs, old_action, images).mean().type(self.FT)
